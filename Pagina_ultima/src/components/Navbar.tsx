...
/* =========================
   Tipos
========================= */
type NavPage = 'home' | 'services' | 'about' | 'contact';

interface NavbarProps {
  onNavigate: (page: NavPage) => void;
  onContactClick: () => void;
  currentPage: NavPage | 'terms' | 'privacy';
}
...
const Navbar: React.FC<NavbarProps> = ({
  ...
  /* =========================
     Navegación
  ========================= */
  const navLinks: { name: string; id: NavPage }[] = [
    { name: 'Inicio', id: 'home' },
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center justify-center space-x-12 flex-[2]">
            {navLinks.map((link) => (
              ...
              >
                {link.name}
                {currentPage === link.id && (
                  <span className="absolute -bottom-2 left-0 h-0.5 w-full bg-[#135bec] rounded-full" />
                )}
              </button>
            ))}
          </div>
          ...
          <div className="lg:hidden">
            <button
              ...
              className={isSolid ? 'text-slate-900' : 'text-white'}
            >
